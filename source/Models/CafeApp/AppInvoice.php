...
class AppInvoice extends Model
{
    ...
    public function launch(User $user, array $data): ?AppInvoice
    {
        ...
        $periodList = ["month", "year","week"];
        if (!in_array($data["period"], $periodList)) {
            $this->message->error("O período de cobrança da fatura deve ser semanal, mensal ou anual");
            return null;
        }
    ...
    public function filter(User $user, string $type, ?array $filter, ?int $limit = null): ?array
    {
        ...
        $due_week = (!empty($filter["date"]) ? explode("-", $filter["date"])[2] : date("d"));
        $due_at = "AND (year(due_at) = '{$due_year}' AND month(due_at) = '{$due_month}' AND week(due_at) = '{$due_week}')";
