...
class App extends Controller
{
    ...
    public function cars(?array  $data): void
    {
        //create car
        if (!empty($data["car"]) && !empty($data["car_name"])){

            $car = new AppCar();
            $car->user_id = $this->user->id;
            $car->car = filter_var($data["car_name"], FILTER_SANITIZE_STRIPPED);
            $car->save();
        ...
        if (!empty($data["car"]) && !empty($data["car_edit"])){
            $car = (new AppCar())->find("user_id = :user AND id = :id",
                "user={$this->user->id}&id={$data["car"]}")->fetch();

            if ($car) {
                $car->car = filter_var($data["car_edit"], FILTER_SANITIZE_STRIPPED);
                $car->save();
            }
            echo json_encode(["car_edit" => true]);
            return;
        }

        //delete car
        if (!empty($data["car"]) && !empty($data["car_remove"])) {
            $car = (new AppCar())->find("user_id = :user AND id = :id",
                "user={$this->user->id}&id={$data["car"]}")->fetch();

            if ($car) {
                $car->destroy();
                (new Session())->unset("carfilter");
            }

            echo json_encode(["car_remove" => true]);
            return;
        }

        $head = $this->seo->render(
            "Meus Carros - " . CONF_SITE_NAME,
            CONF_SITE_DESC,
            url(),
            theme("/assets/images/share.jpg"),
            false
     ...
     * @param array $data
     */
    public function launchService(array $data): void
    {
        if (request_limit("applaunch", 20, 60 * 5)) {
            $json["message"] = $this->message->warning("Foi muito rápido {$this->user->first_name}! Por favor aguarde 5 minutos para novos lançamentos.")->render();
            echo json_encode($json);
        ...
        $data["value"] = (!empty($data["value"]) ? str_replace([".", ","], ["", "."], $data["value"]) : 0);
        if (!$service->launchService($this->user, $data)) {
            $json["message"] = $service->message()->render();
            echo json_encode($json);
            return;
        }

        $type = ($service->type == "service" ? "troca de óleo" : "serviço|Reparo");
        $this->message->success("Tudo certo, sua {$type} foi lançada com sucesso")->flash();
    ...
    public function profile(?array $data): void
    {
        if (!empty($data["update"])) {
            ...
            list($dd, $mm, $yy) = explode("/", $data["cnhval"]);
            $user = (new User())->findById($this->user->id);
            $user->first_name = $data["first_name"];
            $user->last_name = $data["last_name"];
            $user->genre = $data["genre"];
            $user->datebirth = "{$y}-{$m}-{$d}";
            $user->cnhval = "{$yy}-{$mm}-{$dd}";
            $user->document = preg_replace("/[^0-9]/", "", $data["document"]);
            $user->cnhdoc = preg_replace("/[^0-9]/", "", $data["cnhdoc"]);
    ...
    public function services_home(?array $data): void
    {
        $head = $this->seo->render(
            "Manutenções - " . CONF_SITE_NAME,
            CONF_SITE_DESC,
            url(),
            theme("/assets/images/share.jpg"),
            false
        );
        echo $this->view->render("services_home", [
            "head" => $head,
