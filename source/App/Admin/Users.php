...
class Users extends Admin
{
    ...
    public function user(?array $data): void
    {
        ...
        if (!empty($data["action"]) && $data["action"] == "create") {
            ...
            $userCreate->cnhval = date_fmt_back($data["cnhval"]);
            $userCreate->document = preg_replace("/[^0-9]/", "", $data["document"]);
            $userCreate->cnhdoc = preg_replace("/[^0-9]/", "", $data["cnhdoc"]);
        ...
        if (!empty($data["action"]) && $data["action"] == "update") {
            ...
            $userUpdate->cnhval = date_fmt_back($data["cnhval"]);
            $userUpdate->document = preg_replace("/[^0-9]/", "", $data["document"]);
            $userUpdate->cnhdoc = preg_replace("/[^0-9]/", "", $data["cnhdoc"]);
        ...
        if (!empty($data["user_id"])) {
            ...
            $userUpdate = (new User())->findById($userId);
        }

        $head = $this->seo->render(
            CONF_SITE_NAME . " | " . ($userUpdate ? "Perfil de {$userUpdate->fullName()}" : "Novo Usuário"),
            CONF_SITE_DESC,
            url("/admin"),
            url("/admin/assets/images/image.jpg"),
            false
